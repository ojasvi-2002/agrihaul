...
It does **not** cover the original prototype (`index.html`, `css/`, `js/`, the root `README.md`). That's a separate, untouched legacy codebase — see [`docs/CURRENT_SYSTEM.md`](./CURRENT_SYSTEM.md) if you need to understand it. The two live side by side in this repo but share no code.
...
agrihaulapp/
│
├── index.html, css/, js/, README.md    ← OLD PROTOTYPE. Untouched. Ignore for new work.
│
...
- [ ] **Decide on a hosting provider** for both frontend and backend — nothing in the code assumes one, but you'll need to actually pick one and deploy.
